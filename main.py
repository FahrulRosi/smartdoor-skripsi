...
class SmartDoorApp:
    ...
    def __init__(self):
        ...
        self.matcher = FaceMatcher(config.MATCH_THRESHOLD)
    ...
    def _set_ui(self, status, color, instr="", wait=False, bbox=None):
        self.ui.update({"status": status, "color": color, "instr": instr, "wait": wait, "bbox": bbox if bbox else self.ui["bbox"]})
    ...
    def _load_memory(self):
        try:
            ...
            UIHelper.log(f"Gagal muat: {e}", "WARNING")
    ...
    def _reset_state(self):
        ...
        self.wait_center, self.center_hold, self.blink_passed, self.blink_hold, self.ear_hist = False, 0, False, 0, []
    ...
    def _check_action(self, action, face):
        if action == "BLINK": 
            val = UIHelper.get_ear(face)
            self.ear_hist.append(val)
            if len(self.ear_hist) > 5: self.ear_hist.pop(0)
            
            target = getattr(self, 'dyn_blink_thr', 0.20)
            smooth = sum(self.ear_hist) / len(self.ear_hist)
            
            if smooth <= target: 
                self.blink_hold += 1
            else:
                if self.blink_hold >= 1: 
    ...
    def _ai_worker(self):
        while self.running:
            ...
            if not faces: 
                self._reset_state()
                self._set_ui("", config.COLOR_WHITE, wait=True, bbox=None)
            else: 
                self._set_ui("", config.COLOR_WHITE, wait=False, bbox=faces[0].bbox)
    ...
    def _process_face(self, raw, enhanced, face):
        if face.bbox[3] > int(config.FRAME_HEIGHT * 0.50): 
            self._reset_state()
            self._set_ui("TERLALU DEKAT", config.COLOR_YELLOW, "Mundur sedikit")
            return
        ...
        if self.state in (ValidationState.IDLE, ValidationState.RECOGNIZING):
            ...
            if not sp.get("real", True):
                ...
                self._set_ui("SPOOFING", config.COLOR_RED)
                return
            else: 
                self.fake_frames = 0
        ...
        if self.state == ValidationState.RECOGNIZING:
            ...
            if match.get("matched", False):
                ...
                self.reg_pose = [curr.get("yaw", 0), curr.get("pitch", 0), curr.get("roll", 0)]
                ...
                self._set_ui("TIDAK DIKENAL", config.COLOR_RED)
            ...
            if self.wait_center:
                ...
                if abs(curr["yaw"]) < 15 and abs(curr["pitch"]) < 15 and abs(curr["roll"]) < 15:
                    ...
                    if self.center_hold >= 10: 
                        ...
                        ec = float(cfg.get("blink_closed", 0.15))
                        eo = float(cfg.get("blink_open", 0.30))
                        self.dyn_blink_thr = ec + ((eo - ec) * 0.40)
                        
                        if self.dyn_blink_thr >= eo or self.dyn_blink_thr <= ec or ec == eo: 
                            self.dyn_blink_thr = getattr(config, 'BLINK_EAR_THRESHOLD', 0.21)
                else: 
                    self.center_hold = 0
                self._set_ui(f"User: {self.last_name}", config.COLOR_CYAN, "Tatap LURUS ke kamera dulu...")
                return 
                
            act = self.seq[self.step_idx]
            inst = self.CHALLENGES[act]
            self._set_ui(f"User: {self.last_name}", config.COLOR_CYAN, f"Tahap {self.step_idx+1}/{len(self.seq)}: {inst}")
            ...
            if self.print_counter % 3 == 0:
                mtrc = f"EAR Mata: {val:.2f}" if act == "BLINK" else f"Sudut: {val:.1f}°"
                print(f"\r[{inst}] {mtrc} | Target: {tgt:.2f} | Latency: {getattr(self, 'latency', 0):.1f}ms      ", end="", flush=True)
            # ==============================================================

            if passed:
                print() # Buat garis baru agar tidak tertimpa baris update Real-time
                ...
                if self.step_idx < len(self.seq): 
                    ...
                    rs, thr, exc = self.match_score, getattr(config, 'MATCH_THRESHOLD', 0.68), 0.80 
                    acc = 100.0 if rs >= exc else (95.0 + (rs - thr) * 4.9 / (exc - thr) if rs >= thr else rs * 100.0)
                    UIHelper.log(f"🔓 AKSES DIBERIKAN. Akurasi: {acc:.1f}% | Waktu: {time.time() - self.auth_start:.2f}s", "SUCCESS")
            ...
            self._set_ui(f"SELAMAT DATANG, {self.last_name}", config.COLOR_GREEN)
        ...
        try:
            while self.running:
                ...
                if self.ui["wait"]: 
                    ...
                    x, y, w, h = self.ui["bbox"]
                    col = self.ui["color"]
                    cv2.rectangle(display, (x, y), (x+w, y+h), col, 3)
                    txt_w = cv2.getTextSize(self.ui["status"], cv2.FONT_HERSHEY_SIMPLEX, 0.65, 2)[0][0]
                    cv2.rectangle(display, (x, y-35), (x + txt_w + 15, y-5), col, -1)
                    cv2.putText(display, self.ui["status"], (x+8, y-12), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2)
                    if self.ui["instr"]: 
                        cv2.putText(display, self.ui["instr"], (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, config.COLOR_YELLOW, 2)
